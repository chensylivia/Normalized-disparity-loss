...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    from model.PSMNet import PSMNet
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_train,allEPE_train,allD1E_train,all3px_train= train_disp(train_loader, model,optimizer, epoch)
        if main_process():
...
def train_disp(train_loader, model, optimizer, epoch):
    ...
    for i, (left_input, right_input,disp_target) in enumerate(train_loader):
        ...
        loss,output_disp= model(left_input,right_input,disp_target)
        optimizer.zero_grad()
    ...
    if main_process():
        logger.info('Train result at epoch [{}/{}]: allEPE/allE1D/all3px {:.4f}/{:.4f}/{:.4f}'.format(epoch+1, args.epochs, allEPE,allD1E,all3px))
    
    return loss_meter.avg,allEPE,allD1E,all3px
