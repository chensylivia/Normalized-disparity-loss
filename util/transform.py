...
class Compose_disp(object):
    # Composes segtransforms: segtransform.Compose([segtransform.RandScale([0.5, 2.0]), segtransform.ToTensor()])
    def __init__(self, disptransform):
        self.disptransform = disptransform

    def __call__(self, left_image, right_image, label):
        for t in self.disptransform:
            left_image, right_image, label = t(left_image, right_image, label)
        return left_image, right_image, label

class Compose_disp_test(object):
    # Composes segtransforms: segtransform.Compose([segtransform.RandScale([0.5, 2.0]), segtransform.ToTensor()])
    def __init__(self, disptransform):
        self.disptransform = disptransform
...
class ToTensor_disp(object):
    # Converts numpy.ndarray (H x W x C) to a torch.FloatTensor of shape (C x H x W).
    def __call__(self, left_image, right_image, label):
        if not isinstance(left_image, np.ndarray) or not isinstance(label, np.ndarray) or not isinstance(right_image, np.ndarray):
            raise (RuntimeError("disptransform.ToTensor() only handle np.ndarray"
                                "[eg: data readed by cv2.imread()].\n"))
        if len(left_image.shape) > 3 or len(right_image.shape) > 3 or len(left_image.shape) < 2 or len(right_image.shape) < 2:
            raise (RuntimeError("disptransform.ToTensor() only handle np.ndarray with 3 dims or 2 dims.\n"))
        ...
        if not len(label.shape) == 2:
            raise (RuntimeError("disptransform.ToTensor() only handle np.ndarray label with 2 dims.\n"))

        left_image = torch.from_numpy(left_image.transpose((2, 0, 1)))
        right_image = torch.from_numpy(right_image.transpose((2, 0, 1)))
        if not isinstance(left_image, torch.FloatTensor):
            left_image = left_image.float()
        if not isinstance(right_image, torch.FloatTensor):
            right_image = right_image.float()
        label = torch.from_numpy(label)
        if not isinstance(label, torch.FloatTensor):
            label = label.float()
        return left_image, right_image, label

class Normalize_disp(object):
    ...
    def __call__(self, left_image,right_image, label):
        if self.std is None:
            for t, m in zip(left_image, self.mean):
                t.sub_(m)
            for t, m in zip(right_image, self.mean):
                t.sub_(m)
        else:
...
class Crop_disp(object):
    ...
    def __call__(self, left_image, right_image, label):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            left_image = cv2.copyMakeBorder(left_image, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.padding)
            right_image = cv2.copyMakeBorder(right_image, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.padding)
        ...
        left_image = left_image[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
        right_image = right_image[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
